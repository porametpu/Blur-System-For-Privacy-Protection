...
with open("frontend/src/components/RecognitionResults.tsx", "r") as f:
    content = f.read()

# Update props interface
new_interface = """interface RecognitionResultsProps {
  persons: DetectedPerson[];
  onViewTimeline: (person: DetectedPerson) => void;
  selections?: Map<number, boolean>;
  onToggleSelection?: (id: number) => void;
}"""
content = re.sub(r'interface RecognitionResultsProps \{.*?\}', new_interface, content, flags=re.DOTALL)

# Update component signature
content = content.replace(
    "export default function RecognitionResults({ persons, onViewTimeline }: RecognitionResultsProps) {",
...
with open("frontend/src/components/RecognitionResults.tsx", "w") as f:
    f.write(content)
